                ...
                if (index_finger_tip.y < index_finger_pip.y and
                    ...
                    if time.time() - finger_up_start >= DELAY_THRESHOLD:
                        ...
                        if len(current_stroke) >= 3:
                            smooth_current = create_smooth_curve(current_stroke, smoothing=15)
                    ...
                    if recording:
                        # 当笔画结束时，创建平滑曲线并保存
                        if len(current_stroke) >= 3:
                            smooth_stroke = create_smooth_curve(current_stroke, smoothing=15)
                        ...
                        # 当达到延迟时间后执行识别操作
                            ...
                            for stroke in all_strokes:
                                ...
                                for i in range(1, len(mirrored_stroke)):
                                    cv2.line(white_image, mirrored_stroke[i-1], mirrored_stroke[i], (0, 0, 0), 2)
