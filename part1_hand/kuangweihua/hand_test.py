      ...
      for hand_landmarks in results.multi_hand_landmarks:
        ...
        if (index_finger_tip.y < index_finger_pip.y and
            ...
            if finger_direction < -0.08:
                ...
                if elapsed_time < save_delay:
                    ...
                    # 显示保存提示（文字位置镜像）
                    cv2.putText(image, "Saving...", (w - (center[0] + 40), center[1] - 40),
                ...
                # 当达到延迟时间后执行保存操作
                    ...
                    cv2.imwrite(f'hand_landmarks_{image_count}.png', white_image)
                    image_count += 1
                    saved = True
                    # 在右上角显示保存成功信息
        ...
        # 判断是否食指、中指和无名指都伸直（删除手势）
            ...
            if elapsed_time < delete_delay:
                ...
                # 显示删除提示（文字位置镜像）
                cv2.putText(image, "Deleting...", (w - (center[0] + 40), center[1] - 40),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

            # 当达到延迟时间后执行删除操作
            elif delete_confirmed:
                files = sorted([f for f in os.listdir('.') if f.startswith('hand_landmarks_')])
                if files:
                    last_file = files[-1]
                    try:
                        os.remove(last_file)
        ...
        # 检测大拇指点赞手势来结束程序
            ...
            if elapsed_time < exit_delay:
                ...
                # 显示退出提示（文字位置镜像）
                cv2.putText(image, "Exiting...", (w - (center[0] + 40), center[1] - 40),
            ...
            # 当达到延迟时间后退出程序
                ...
                if all_strokes:
                    ...
                    # 只绘制平滑后的笔画（镜像处理）
                    for stroke in all_strokes:
                        mirrored_stroke = [(w - x, y) for x, y in stroke]  # 镜像每个点的x坐标
                        for i in range(1, len(mirrored_stroke)):
                            cv2.line(white_image, mirrored_stroke[i-1], mirrored_stroke[i], (0, 0, 255), 2)
                    cv2.imwrite(f'hand_landmarks_{image_count}.png', white_image)
                    print("Final drawing saved before exit")
